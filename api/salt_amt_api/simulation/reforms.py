"""Reform parameter generation for PolicyEngine-US simulations."""

import numpy as np
from typing import Optional

# Current Policy parameters (TCJA extended)
...
def get_other_tcja_provisions() -> dict:
...
class PolicyReforms:
    """Helper class for generating policy reform dictionaries."""

    @staticmethod
    def policy_reforms(reform_params: dict) -> dict:
        """Generate reform dictionary for SALT cap and AMT adjustments."""
...
def get_reform_params_from_config(policy_config: dict) -> dict:
    """Convert policy configuration to reform parameters."""
    reform_params = {
        "salt_phase_out_enabled": False,
        "salt_phase_out_rate": 0,
        "salt_phase_out_threshold_joint": 0,
    ...
    # Handle SALT cap
    salt_cap = policy_config.get("salt_cap", "Current Law (Uncapped)")
    if salt_cap == "Current Law (Uncapped)":
    ...
    if policy_config.get("amt_repealed"):
        ...
        if (
            policy_config.get("amt_exemption")
