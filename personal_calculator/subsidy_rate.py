...
def calculate_subsidy_rate(situation, period, policy_config):
    ...
    current_policy_base = (
        current_law_base + current_policy_results["reform_current_policy_impact"]
    )
    ...
    mod_current_policy_results = calculate_impacts(
        modified_situation, CURRENT_POLICY_PARAMS
    )
    current_policy_mod = (
        current_law_mod + mod_current_policy_results["reform_current_policy_impact"]
    )

    mod_your_policy_results = calculate_impacts(modified_situation, your_policy_params)
    your_policy_mod = (
        current_law_mod + mod_your_policy_results["selected_reform_impact"]
    )
